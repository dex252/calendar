...
namespace Calendar
{
    internal class Generator
    {
        ...
        public Generator(Day[] days, int maxLessons, List<UnicLesson> unicLessons, List<Generations> generations, double mainMark)
        {
            main = new Day[6];
        ...
        public void GetPopulations(int numPopulations)//число популяций
        {
            ...
            for (int j = 0; j < numPopulations; j++)
            {
                ...
                for (int i = 0; i < 15; i++)
                {
                    ...
                    /*здесь ошибка*/
      /*меняет ссылу в main*/              GetNewGeneration(main, i);//вызов скрещивания особи для составления нового поколения
                    //Mutations();
        ...
        private void Swap(int leftIndex, int rightIndex)//меняем местами гены у указанных дней
        {
            ...
            /*
             * Адекватно влипить сюда мутацию, но тут ошибка по передаче ссылки а не значения
             */
